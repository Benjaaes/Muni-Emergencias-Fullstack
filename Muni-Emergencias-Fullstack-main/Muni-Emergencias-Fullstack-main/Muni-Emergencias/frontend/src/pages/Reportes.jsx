...
const ReportesPage = () => {
  ...
  const enviarReporte = async (e) => {
    ...
    try {
      const res = await fetch(API_URL, {
        ...
        body: JSON.stringify({ tipoEmergencia: 'Incendio Forestal', descripcion: descripcion, estado: 'ACTIVO' })
      });
      if (res.ok) { setDescripcion(''); cargarReportes(); }
    } catch (err) {
  ...
  const getStatusStyle = (estado) => {
    const base = { padding: '4px 12px', borderRadius: '20px', fontSize: '0.75rem', fontWeight: 'bold', display: 'inline-block' };
    if (estado === 'CONTROLADO') return { ...base, backgroundColor: '#064e3b', color: '#34d399', border: '1px solid #059669' };
    if (estado === 'CRÍTICO') return { ...base, backgroundColor: '#7f1d1d', color: '#fca5a5', border: '1px solid #dc2626' };
    return { ...base, backgroundColor: '#78350f', color: '#fbbf24', border: '1px solid #d97706' };
  };
    ...
    <div style={styles.appContainer}>
      <nav style={styles.navbar}>
        ...
        <div style={styles.navMenu}>
          <span style={styles.navLink} onClick={() => navigate('/')}>CERRAR SESIÓN</span>
          <span style={{ ...styles.navLink, ...styles.navLinkActive }}>REPORTES DE INCENDIO</span>
          <span style={styles.navLink} onClick={() => navigate('/alertas')}>ALERTAS ACTIVAS</span>
        </div>
      </nav>
            ...
            <div style={styles.panelBody}>
              ...
              <form onSubmit={enviarReporte}>
                ...
                <div style={styles.inputGroup}>
                  ...
                  <textarea style={styles.textarea} value={descripcion} onChange={(e) => setDescripcion(e.target.value)} required />
                </div>
                ...
                {reportes.length === 0 ? (
                  ...
                  reportes.map((r, index) => (
                    <tr key={r.id} style={index % 2 === 0 ? styles.trEven : styles.trOdd}>
                      ...
                      <td style={styles.td}><span style={getStatusStyle(r.estado)}>{r.estado === 'CONTROLADO' ? '✓ ' : '⚠️ '}{r.estado}</span></td>
...
const styles = {
  appContainer: { backgroundColor: '#0a0a0a', minHeight: '100vh', width: '100%', color: '#e5e5e5', fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif", overflowX: 'hidden', boxSizing: 'border-box' },
  navbar: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#171717', padding: '0 40px', height: '70px', borderBottom: '1px solid #262626' },
  ...
  navLink: { fontSize: '0.85rem', fontWeight: '600', color: '#a3a3a3', cursor: 'pointer', display: 'flex', alignItems: 'center' },
  ...
  textarea: { width: '100%', boxSizing: 'border-box', backgroundColor: '#111', border: '1px solid #fbbf24', padding: '12px 15px', borderRadius: '4px', color: '#fff', fontSize: '0.9rem', minHeight: '100px', resize: 'none', outline: 'none' },
  btnSubmit: { width: '100%', padding: '15px', background: 'linear-gradient(90deg, #dc2626 0%, #ea580c 100%)', color: '#fff', border: 'none', borderRadius: '6px', fontSize: '1rem', fontWeight: 'bold', cursor: 'pointer', boxShadow: '0 4px 15px rgba(220, 38, 38, 0.4)' },
  ...
  iconBtn: { background: 'none', border: 'none', cursor: 'pointer', fontSize: '1.2rem', opacity: 0.6 },
